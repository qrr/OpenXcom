...
{
	return std::fabs(l-r) <= FLT_EPSILON * std::fmax(1.0f, std::fmax(std::fabs(l), std::fabs(r)));
}

inline bool AreSame(double l, double r)
{
	return std::fabs(l-r) <= DBL_EPSILON * std::fmax(1.0, std::fmax(std::fabs(l), std::fabs(r)));
}
